...
import { POS_COMPETITORS } from '../config/competitors';
...
const POSNewsScreen = () => {
  ...
  const loadNews = async (isRefresh = false) => {
    try {
      ...
      // Fetch news for all POS competitors
      const rawArticles = await fetchNewsForAllCompetitors(POS_COMPETITORS);
      ...
      const analyses = {};
      for (const competitor of POS_COMPETITORS) {
  ...
  if (loading) {
    return <LoadingSpinner message="Loading POS news..." />;
  }
  ...
  return (
    ...
    >
      <View style={styles.header}>
        <Text style={styles.headerTitle}>talech Competitor Intelligence</Text>
        <Text style={styles.headerSubtitle}>
          Tracking strategic moves from key POS competitors
        </Text>
      </View>
      ...
      {POS_COMPETITORS.map((competitor) => (
        <CompetitorSection
          key={competitor.name}
